...
_PLAYER_OVERRIDES: dict[str, str] = {
    **_PITCHER_OVERRIDES,
}
...
def _has_kana(text: str) -> bool:
    return bool(_KATAKANA_RE.search(text) or _HIRAGANA_RE.search(text))
...
def display_pitcher_name(raw: str | None) -> str | None:
    ...
    override = _apply_overrides(raw, _PLAYER_OVERRIDES)
    if override:
        return override

    if _has_kana(raw):
        # Convert hiragana to katakana then romaji for foreign / kana names.
        kana = "".join(
            chr(ord(ch) + 0x60) if "ぁ" <= ch <= "ゖ" else ch for ch in raw
        )
        return _katakana_to_romaji(kana)

    # Kanji names are shared with Chinese orthography; keep as-is.
...
def display_player_name(raw: str | None) -> str | None:
    """Batter / pitcher name for UI: Chinese kanji or English (never katakana)."""
    return display_pitcher_name(raw)


def display_position(raw: str | None) -> str | None:
    if not raw:
        return raw
    text = raw.strip().strip("()（）")
    # Fix occasional scrapes like "右)左"
    text = text.replace(")", "").replace("（", "").replace("）", "")
    mapped = _POSITION_OVERRIDES.get(text)
    if mapped:
        return mapped
    # Compound roles e.g. 走一
    for key, value in sorted(_POSITION_OVERRIDES.items(), key=lambda item: -len(item[0])):
        if key in text:
...
def localize_starting_lineups(lineups: dict | None) -> None:
    if not lineups:
        return
    for side in ("away", "home"):
        ...
        if not isinstance(block, dict):
            continue
        localize_probable_pitcher(block.get("opposingPitcher"))
        for batter in block.get("batters") or []:
            if not isinstance(batter, dict):
                continue
            if batter.get("name"):
                batter["name"] = display_player_name(batter["name"])
            if batter.get("position"):
                batter["position"] = display_position(batter["position"])
            if batter.get("positionLabel"):
                batter["positionLabel"] = display_position(batter["positionLabel"])
...
def localize_matchup_payload(payload: dict) -> None:
    ...
    localize_starting_lineups(payload.get("startingLineups"))

...
def _cache_looks_thin(players: dict[str, Any]) -> bool:
    """Incomplete builds often stop after a handful of games (max AB stays tiny)."""
    if not players:
        return True
    max_ab = 0
    for bucket in players.values():
        try:
            max_ab = max(max_ab, int((bucket or {}).get("ab") or 0))

"""Fallback probable pitchers from PTT BaseballXXXX daily posts."""
...
PTT_SEARCH = "https://www.ptt.cc/bbs/BaseballXXXX/search?q=%E5%85%88%E7%99%BC%E6%8A%95%E6%89%8B"
PTT_HEADERS = {
    "User-Agent": (
...
_ARTICLE_LINK_RE = re.compile(
    r'<a href="(/bbs/BaseballXXXX/M\.[^"]+)">(\[[^\]]+\]\s*)?CPBL\s+\d+/\d+\s+先發投手\s*</a>'
)
...
async def fetch_ptt_probable_pitchers(http: httpx.AsyncClient) -> list[dict[str, Any]]:
    ...
    if not article_path:
        for match in re.finditer(
            r'<a href="(/bbs/BaseballXXXX/M\.[^"]+)">([^<]+)</a>', index.text
        ):
            title = match.group(2)

...
const EXPECTED_CACHE_VERSION = 17;
...
function buildContentFingerprint(data) {
  // Ignore cache timestamps so background refresh does not rebuild DOM and reset scroll.
  const { matchup, away, home } = data;
  return JSON.stringify({ matchup, away, home });
}
...
function updateMatchupHeader(data) {
  ...
  if (metaEl) {
    metaEl.textContent = MatchupMeta.buildMetaText(matchup, away, home);
  }
}
      ...
      <table class="data-table games-table">
        <thead>
          <tr>
            ...
            <th class="col-num"><span class="th-full">用球數</span><span class="th-short">球數</span></th>
        ...
        <tbody>
          ${games
            ...
            <tr>
              ...
              <td class="col-num">${MatchupMeta.formatPitchCount(game.pitchCount)}</td>
...
function renderMatchup(data, { skipIfUnchanged = false } = {}) {
  ...
  // While user is swiping tables, only refresh the header — full DOM rebuild snaps scroll back.
...
async function loadTeams() {
  ...
  teamSelect.value = fromQuery || savedTeam || String(PHILADELPHIA_ID);
}
...
function setBusy(isBusy, message) {
  showLoading(isBusy && !hasDisplayedData);
...
function needsFreshData(data, games) {
  const hasPanels =
    (data.away?.games?.length ?? 0) >= Math.min(5, games) &&
    (data.home?.games?.length ?? 0) >= Math.min(5, games);
  if (data.refreshing && hasPanels && !teamGamesMissingScores(data)) {
    return false;
  }
...
function beginTeamSwitch() {
  ...
  const aRoot = document.getElementById("a-table-root");
  const sRoot = document.getElementById("situational-root");
  if (aRoot) aRoot.innerHTML = "";
...
teamSelect.addEventListener("change", () => {
  beginTeamSwitch();
  fetchAnalysis(false, true, false, true);
});

...
async function loadTeams() {
  const { resp, data: teams } = await ApiUtils.fetchJson("/api/cpbl/teams", fetchWithTimeout, {
    onWaiting(n, total) {
...
async function fetchAnalysis(force = false, allowAutoRetry = true, isPoll = false) {
  ...
  try {
    ...
    const { resp, data } = await ApiUtils.fetchJson(`/api/cpbl/matchup?${query}`, fetchWithTimeout, {
      onWaiting(n, total) {
        cacheStatusEl.textContent = `雲端啟動中…（${n}/${total}，約 30～60 秒）`;
      },
    });
